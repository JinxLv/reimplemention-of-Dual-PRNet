...
from .base_networks import  VTNAffineStem,DUAL
...
class RecursiveCascadedNetworks(Network):
    ...
    def build(self, img1, img2, seg1, seg2, pt1, pt2):
        ...
        def GetSimilarityLoss(img_fixed,img_moving):
        ...
        for stem_result, (stem, params) in zip(stem_results, self.stems):
            if 'W' in stem_result:
                ...
                if params['raw_weight'] > 0:
                    stem_result['raw_loss'] = self.similarity_loss(img1,stem_result['warped'])
        ...
        mean_mse = tf.reduce_mean(tf.abs(warped*255-img1*255))
    ...
    def jacobian_det(self, flow):
        ...
        return tf.sqrt(var) #return the std of the jacb det 
