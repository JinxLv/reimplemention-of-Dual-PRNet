...
def main():
    ...
    for val_subset in val_subsets:
        if args.val_subset is not None:
            output_fname = path_prefix + '-' + str(val_subset) + '_atlas2.txt'
        else:
            output_fname = path_prefix + '_atlas2.txt'
            output_xls = path_prefix + '_atlas2.xls'
        with open(output_fname, 'w') as fo:
            ...
            ##################image save#########################
            image_save_path = os.path.join('./test_images', short_name(checkpoint)+'_atlas2')#+'_onebyone'
            ...
            for i in range(len(results['dices'])):
                print(results['id1'][i],results['dices'][i],results['id2'][i], np.mean(results['jaccs'][i]), np.mean(results['landmark_dists'][i]), results['jacobian_det'][i], file=fo)#
                for j in range(len(results['dices'][i])):
                     testSheet1.write(i,j,float('%.4f'%results['dices'][i][j]))
            writebook.save(output_xls) 
            print('Summary', file=fo)
            jaccs, dices, landmarks,ncc,mse = results['jaccs'], results['dices'], results['landmark_dists'],results['total_ncc'],results['total_mse']
            ...
            print("mse score: {} ({})".format(np.mean(mse), np.std(
                np.mean(mse, axis=-1))), file=fo)
